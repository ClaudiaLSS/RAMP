...
**|fixed|**     `Appliance.maximum_profile` is now derived from a dedicated `windows_mask` attribute rather than from `daily_use`, which is overwritten with the realised load of the day being simulated. Recomputing `UseCase.calc_peak_time_range` (directly, via a second `initialize`, by assigning `peak_enlarge`, or by building a new `UseCase` from already-simulated users, as a parameter sweep does) previously read a realised profile instead of the windows of use and rescaled it by the power a second time, returning a peak window shifted by hours. `calc_peak_time_range` also no longer raises `IndexError` when the theoretical maximum is attained at a single minute. Profiles generated by a use case which computes its peak time range once, before generating, are unaffected.

**|new|**       Household-level occupancy mask, enabled by giving a `User` a `prob_home` (probability of the household being present on a given day). A single presence draw is made per household per day and gates all of that household's appliances together, so an absent household produces zero load that day. Presence and `occasional_use` are independent and multiplicative, `P(appliance runs) = prob_home * occasional_use`, which makes `occasional_use` a fraction of *home* days once the mask is active. The feature is opt-in: with `prob_home=None` (the default) no random number is drawn and results are unchanged. Not supported with `parallel_processing=True`, and not part of the .xlsx model format.
